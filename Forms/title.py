...
from FormUI.ui_title import Ui_titleWD
...
class TitleWidget(QWidget, Ui_titleWD):
    ...
    def slot_checked(self):
        for b in self.button_group.buttons():
            # name=unicode(b.text()).lower()  # 有可能翻译为中文, 所以别取text
    ...
    def eventFilter(self, obj, event):
        """返回True, 不传递事件; 返回False继续传递(传递到子控件本身的event,然后子控件本身的具体事件))
        配合installEventFilter()使用
        """
